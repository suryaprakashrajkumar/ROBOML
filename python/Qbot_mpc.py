...
frameRate, sampleRate = 60.0, 1/60
...
wrwlmax=8     #maximum wheel speed [rad/s]

eta = 1.3
alpha = 10.5
N = 30  # Prediction horizon
Q_cost = np.diag([100, 100, 10])
R_cost = np.diag([10, 10])
...
while elapsed_time() < simulationTime: 
    ...
    # # px = moving_average(px,window_size=5)
    # # px = moving_average(py,window_size=5)
    # dx = np.diff(px)
    # dy = np.diff(py)
    # theta = np.arctan2(dy, dx)
    # theta - np.unwrap(theta)
    # theta = moving_average(theta,window_size=5)
    # print(theta[-1])
    thetan = -k[8]
    theta = np.append(theta, thetan)
    theta = np.unwrap(theta)
    waypoints = predict_waypoints(eta, alpha, sampleRate, N+1, start)
    xrh, yrh, _, _, thetarh = waypoints
    print(xrh)
    Q = np.array([k[1][0], -k[1][1], theta[-1]])
    x_opt, u_opt, wl_opt, wr_opt = solve_nmpc(Q, xrh, yrh, thetarh, N, sampleRate, Q_cost, R_cost, wrwlmax, r, d)
    commands, arm, noKill = [u_opt[0][0],u_opt[1][0]], 1, True
    # commands, arm, noKill = [wl_opt[0],wr_opt[0]], 1, True
